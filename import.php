...
if (($handle = fopen($_FILES['csv-file']['tmp_name'], "r")) !== FALSE) {
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
       if ($session_user_id == $data[5]) {
            $stmt = $mysqli->prepare("INSERT INTO products (`name`, code, price, preview_text, detail_text, user_id) VALUES(?,?,?,?,?,?)");
            $stmt->bind_param('siissi', $data[0],$data[1],$data[2],$data[3], $data[4],$data[5]);
            $stmt->execute();
            $row++;
        }
    }
